...
对前端进行全面视觉优化，提升设计质感和交互体验。**零破坏性改动**（不涉及 JS 逻辑），版本升级至 **v3.0**。
...
- `--primary-subtle`、`--accent-glow`、`--shadow-lg`、`--radius-xl`
- `--transition-fast` / `--transition-normal` / `--transition-slow` 统一动效时长

### 动画系统

- 按钮 hover 上浮、active 按下缩放
- Primary 按钮悬浮发光阴影
- Danger 按钮悬浮红色光晕
- Toast 弹性入场动画
- 自定义滚动条
...
- Model Tabs 改为 segmented control 风格
...
> 本次为纯 CSS 视觉优化，无 JS 逻辑改动，版本更新为 v3.0。
